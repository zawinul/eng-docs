...
(function(){
	...
	function initBusinessLogic() {
		...
		camunda.getActivityList(params.id).then(
			...
			function(){
				$('.error-message').text("l'oggetto non esiste, forse è scaduto");
			}
		);
		...
		function avvia() {
			...
			camunda.getInstanceVariables(params.id).then(function(vars){
				...
				$('.params-ko').show();
				$('.error-message').text(notFound?"l'oggetto non esiste, forse è scaduto":'ERROR '+jqXHR.status);
				showJSON(jqXHR);
			});
